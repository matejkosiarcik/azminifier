...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS helper--gitman--base
WORKDIR /app
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS helper--gitman--final
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM debian:12.11-slim AS runtime--nodejs--build1
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        g++ gcc make >/dev/null && \
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS runtime--nodejs--build2
...
# - CXXFLAGS="-flto"
# Compile NodeJS
FROM debian:12.11-slim AS runtime--nodejs--build3
WORKDIR /app
...
FROM debian:12.11-slim AS runtime--nodejs--final
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        moreutils >/dev/null && \
...
FROM debian:12.11-slim AS runtime--ruby--rbenv--final
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
# Install ruby with rbenv
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS runtime--ruby--rbenv--buildplatform--final
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        autoconf bison build-essential ca-certificates curl git moreutils \
...
FROM --platform=$BUILDPLATFORM node:24.2.0-slim AS minifiers--cli--build
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        moreutils \
...
FROM debian:12.11-slim AS minifiers--cli--final
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS minifiers--minifiers--cli--buildplatform--final
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        moreutils nodejs npm \
...
FROM --platform=$BUILDPLATFORM node:24.2.0-slim AS minifiers--nodejs--build1
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS minifiers--nodejs--build2
WORKDIR /app/minifiers
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM debian:12.11-slim AS minifiers--nodejs--final
WORKDIR /app/minifiers
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        moreutils nodejs \
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS minifiers--nodejs--buildplatform--final
WORKDIR /app/minifiers
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        moreutils nodejs \
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS minifiers--python--build1
WORKDIR /app/minifiers
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS minifiers--python--build2
WORKDIR /app/minifiers
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM debian:12.11-slim AS minifiers--python--final
WORKDIR /app/minifiers
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        moreutils python3 \
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS minifiers--python--buildplatform--final
WORKDIR /app/minifiers
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        moreutils python3 \
...
FROM debian:12.11-slim AS minifiers--ruby--build
WORKDIR /app/minifiers
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS minifiers--ruby--buildplatform--build
WORKDIR /app/minifiers
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        build-essential libyaml-0-2 moreutils >/dev/null && \
...
FROM debian:12.11-slim AS minifiers--ruby--final
WORKDIR /app/minifiers
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        moreutils >/dev/null && \
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS minifiers--ruby--buildplatform--final
WORKDIR /app/minifiers
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        moreutils >/dev/null && \
...
FROM debian:12.11-slim AS minifiers--shell--final
COPY ./minifiers/shell /app/minifiers/shell

FROM --platform=$BUILDPLATFORM debian:12.11-slim AS minifiers--shell--buildplatform--final
COPY ./minifiers/shell /app/minifiers/shell
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS minified--helper
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS minifiers--cli--minified
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        moreutils nodejs python3 \
        >/dev/null && \
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS minifiers--config--minified
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        moreutils nodejs python3 \
        >/dev/null && \
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS minifiers--nodejs--minified
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        moreutils nodejs python3 \
        >/dev/null && \
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS minifiers--python--minified
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        moreutils nodejs python3 \
        >/dev/null && \
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS minifiers--ruby--minified
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS minifiers--shell--minified
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        moreutils nodejs python3 \
        >/dev/null && \
...
FROM --platform=$BUILDPLATFORM debian:12.11-slim AS runtime--ruby--rbenv--minified
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        moreutils nodejs python3 \
        >/dev/null && \
